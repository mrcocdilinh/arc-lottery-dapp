...
export default function Dashboard() {
      ...
      try {
        ...
        wonEvents.forEach(e => {
          compiledMegaHistory.push({
            ...
            status: 'WIN',
            detail: `${Number(e.args[2])} Winners sharing`,
  ...
  const handleBuyMegaTicket = async () => {
    if (!signerInstance) return showToast('Please connect your wallet first!', 'error');
    if (megaSelectedNumbers.length !== 3) return showToast('Please select exactly 3 numbers!', 'error');
    try {
      setLoadingState('Sign in Wallet...');
      ...
      setLoadingState('Mining Block...');
      await tx.wait();
      showToast(`Ticket purchased! Your numbers: ${sorted.join(' - ')}`, 'success');
      setMegaSelectedNumbers([]);
      await fetchAllBlockchainData();
    } catch { showToast('Ticket purchase failed.', 'error'); } finally { setLoadingState(''); }
  };
  ...
  const handleDrawMega = async () => {
    ...
    try {
      ...
      await fetchAllBlockchainData();
    } catch { showToast('Draw condition not met (Not time yet or no tickets).', 'error'); } finally { setLoadingState(''); }
  };
    ...
    <div className="min-h-screen bg-[#050810] text-white p-4 md:p-8 font-sans antialiased relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="flex gap-3 p-2 bg-[#0b1221]/80 backdrop-blur-md border border-slate-800/80 rounded-3xl mb-10 overflow-x-auto">
          ...
          <button onClick={() => setActiveTab('ledger')} className={`flex-1 py-3.5 px-6 rounded-2xl font-black text-sm transition-all duration-300 whitespace-nowrap ${activeTab === 'ledger' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : 'text-slate-500'}`}>HISTORY</button>
        </div>
        ...
        {activeTab === 'mega' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fadeIn">
              ...
              <div>
                ...
                <div className="grid grid-cols-6 sm:grid-cols-9 gap-2 p-3 bg-[#050810] rounded-2xl max-h-[180px] overflow-y-auto border border-slate-800/80 mb-6 custom-scrollbar">
                  {Array.from({length: 45}, (_, i) => i + 1).map(num => (
                    <button key={num} onClick={() => toggleMegaNumber(num)} className={`py-2 rounded-xl text-xs font-black transition-all ${megaSelectedNumbers.includes(num) ? 'bg-fuchsia-500 text-white shadow-md' : 'bg-slate-900 text-slate-400 border border-slate-800 hover:text-white'}`}>
            ...
            <div className="bg-[#0b1221]/70 border border-slate-800 rounded-[40px] p-8 shadow-2xl flex flex-col justify-center items-center text-center">
               ...
               <p className="text-slate-400 text-sm max-w-sm leading-relaxed mb-6">80% goes to Jackpot, 10% to Dev, 10% to Seed Pool. If no winner, the Jackpot rolls over to the next round!</p>
               <div className="bg-[#050810] px-6 py-3 rounded-full border border-slate-800 text-xs font-mono font-bold text-slate-400">
                 Tickets bought this round: <span className="text-fuchsia-400">{megaTicketsCountThisRound}</span>
               </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'ledger' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-fadeIn">
            ...
            <div className="bg-[#0b1221]/70 border border-slate-800 rounded-[40px] p-8 shadow-2xl flex flex-col">
              ...
              <div className="bg-[#050810] border border-slate-800 rounded-3xl p-4 overflow-y-auto max-h-[450px] flex-1 space-y-3 custom-scrollbar mt-4">
                  ...
                  classicHistoricalWinners.map((data, index) => (
                    <div key={index} className="bg-[#0b1221] p-4 rounded-xl border border-slate-800/80 flex flex-col gap-2">
                      <div className="flex justify-between items-center">
                        <span className="bg-cyan-500/10 border border-cyan-500/20 text-cyan-400 font-black text-[10px] uppercase px-3 py-0.5 rounded-full">Round #{data.round}</span>
